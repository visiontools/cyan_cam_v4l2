...
void
device_get_available_modes(cam_v4l2_t * cam)
{
	...
	while (0 == xioctl(cam->fd, VIDIOC_ENUM_FMT, &fmtdesc)) {
		...
		case V4L2_FRMSIZE_TYPE_DISCRETE:
			...
			frmsizeenum.index = 0;
			frmsizeenum.pixel_format = fmtdesc.pixelformat;
			while (0 == xioctl(cam->fd, VIDIOC_ENUM_FRAMESIZES,
				&frmsizeenum)) {
	...
	cam->v4l_modes =
	    (v4lmode_t *) malloc(cam->nb_modes * sizeof(v4lmode_t));
